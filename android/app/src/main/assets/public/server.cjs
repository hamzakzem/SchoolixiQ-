...
async function startServer() {
  ...
  const sanitizeForFirestore = (val) => {
    ...
    if (typeof val === "object") {
      ...
      if (val instanceof import_firebase_admin.default.firestore.FieldValue || val instanceof import_firebase_admin.default.firestore.Timestamp || val instanceof import_firebase_admin.default.firestore.GeoPoint || val instanceof import_firebase_admin.default.firestore.DocumentReference) {
        return val;
      }
      if (val.constructor && [
        "FieldValue",
  ...
  const syncUserClaims = async (uid, role, schoolId, permissions) => {
    try {
      ...
      await import_firebase_admin.default.auth().setCustomUserClaims(uid, claims);
  ...
  app.post("/api/upload", verifyToken, import_express.default.json({ limit: "20mb" }), async (req, res) => {
    try {
      ...
      const ext = import_path.default.extname(storagePath).toLowerCase();
...
<dict>
    ...
    try {
      ...
      try {
        ...
        if (password) {
          ...
          if (!hasPassword) {
            updateParams.password = import_crypto.default.randomBytes(16).toString("hex") + "SecureP1!";
        ...
        if (authError.code === "auth/user-not-found") {
          const generatedSecurePass = import_crypto.default.randomBytes(16).toString("hex") + "SecureP1!";
      ...
      if (role === "student" && schoolId) {
        ...
        await db.runTransaction(async (transaction) => {
          ...
          transaction.set(db.collection("users").doc(uid), {
            ...
            schoolId,
            ...additionalData,
            createdAt: isExistingUser ? existingUserData.createdAt : import_firebase_admin.default.firestore.FieldValue.serverTimestamp()
          }, { merge: true });
  ...
  app.post("/api/admin/delete-school", verifyAdmin, async (req, res) => {
    ...
    try {
      ...
      const userDeletions = usersQuery.docs.map(async (uDoc) => {
        try {
          await import_firebase_admin.default.auth().deleteUser(uDoc.id);
        } catch (e) {
        }
  ...
  app.post("/api/admin/backup", verifyAdmin, async (req, res) => {
    ...
    try {
      ...
      for (let i = 0; i < collectionsToBackup.length; i++) {
        ...
        while (hasMore) {
          let query = db.collection(collName).orderBy(import_firebase_admin.default.firestore.FieldPath.documentId()).limit(1e3);
          if (lastDoc) {
  ...
  app.listen(PORT, "0.0.0.0", async () => {
    ...
    try {
      const db = getDb();
      const bootTime = import_firebase_admin.default.firestore.Timestamp.now();
          ...
          if (change.type === "added") {
            ...
            try {
              ...
              if (userTokens.length > 0) {
                ...
                const response = await import_firebase_admin.default.messaging().sendEach(messages);
