...
async function startServer() {
  ...
  app.use((req, res, next) => {
    ...
    if (origin && isAllowed) {
      ...
      return res.status(403).json({ error: 'CORS_BLOCKED', message: '╪║┘è╪▒ ┘à╪│┘à┘ê╪¡ ╪¿╪º┘ä┘ê╪╡┘ê┘ä ┘à┘å ┘ç╪░╪º ╪º┘ä┘à╪╡╪»╪▒ ┘ä┘ä┘ê╪º╪¼┘ç╪⌐ ╪º┘ä╪¿╪▒┘à╪¼┘è╪⌐' });
    }
        ...
        if (schoolDoc.exists) {
          ...
          if (expirationData) {
            ...
            if (expiryDate < new Date()) {
              return res.status(402).json({ error: 'SUBSCRIPTION_EXPIRED', message: '╪º┘å╪¬┘ç╪¬ ╪╡┘ä╪º╪¡┘è╪⌐ ╪º╪┤╪¬╪▒╪º┘â ╪º┘ä┘à╪»╪▒╪│╪⌐' });
            }
          }
        }
      }
  ...
  const verifyToken = async (req: any, res: any, next: any) => {
    ...
    if (!authHeader?.startsWith('Bearer ')) {
      return res.status(401).json({ error: 'Unauthorized', message: '┘è╪▒╪¼┘ë ╪¬╪│╪¼┘è┘ä ╪º┘ä╪»╪«┘ê┘ä ╪ú┘ê┘ä╪º┘ï ┘ä┘ä┘é┘è╪º┘à ╪¿┘ç╪░┘ç ╪º┘ä╪╣┘à┘ä┘è╪⌐' });
    }
    const token = authHeader.split('Bearer ')[1];
    try {
      const decodedToken = await admin.auth().verifyIdToken(token);
      ...
      return res.status(401).json({ error: 'AuthenticationFailed', message: `┘ü╪┤┘ä ╪º┘ä╪¬╪¡┘é┘é ┘à┘å ╪º┘ä┘ç┘ê┘è╪⌐: ${e.message}` });
    }
  };
    ...
    try {
      ...
      if (buffer.length > maxSizeBytes) {
        return res.status(400).json({
          ...
          message: '╪¡╪¼┘à ╪º┘ä┘à┘ä┘ü ┘â╪¿┘è╪▒ ╪¼╪»╪º┘ï. ╪º┘ä╪¡╪» ╪º┘ä╪ú┘é╪╡┘ë ╪º┘ä┘à╪│┘à┘ê╪¡ ╪¿┘ç ┘ç┘ê 10 ┘à┘è╪¼╪º╪¿╪º┘è╪¬.'
        });
      }
      ...
      if (!allowedExtensions.includes(ext)) {
        return res.status(400).json({
          ...
          message: '┘å┘ê╪╣ ╪º┘ä┘à┘ä┘ü ╪║┘è╪▒ ┘à╪»╪╣┘ê┘à ╪╣┘ä┘ë ┘ç╪░┘ç ╪º┘ä┘à┘å╪╡╪⌐ ┘ä╪ú╪│╪¿╪º╪¿ ╪ú┘à┘å┘è╪⌐.'
        });
      }
      ...
      if (disallowedMimeTypes.includes(contentType.toLowerCase())) {
        return res.status(400).json({
          ...
          message: '╪║┘è╪▒ ┘à╪│┘à┘ê╪¡ ╪¿╪▒┘ü╪╣ ┘à┘ä┘ü╪º╪¬ ╪¿╪▒┘à╪¼┘è╪⌐ ╪ú┘ê ╪╡┘ü╪¡╪º╪¬ ┘ê┘è╪¿ ┘å┘ç╪º╪ª┘è╪º┘ï.'
        });
      }
...
<dict>
  ...
  <array>
    <dict>
      ...
      <string>┘à┘å╪╡╪⌐ SchoolixiQ ╪º┘ä╪¬╪╣┘ä┘è┘à┘è╪⌐</string>
  ...
  app.post('/api/admin/create-user', verifyAdmin, async (req: any, res: any) => {
    try {
      ...
      if (!email) {
        return res.status(400).json({ error: 'EMAIL_REQUIRED', message: '╪º┘ä╪¿╪▒┘è╪» ╪º┘ä╪Ñ┘ä┘â╪¬╪▒┘ê┘å┘è ┘à╪╖┘ä┘ê╪¿' });
      }
      ...
      if (!allowedAdminRoles.includes(req.user.role)) {
        return res.status(403).json({ error: 'FORBIDDEN', message: '╪║┘è╪▒ ┘à╪╡╪▒╪¡ ┘ä┘â ╪¿╪º┘ä┘é┘è╪º┘à ╪¿┘ç╪░┘ç ╪º┘ä╪╣┘à┘ä┘è╪⌐ ╪º┘ä╪Ñ╪»╪º╪▒┘è╪⌐' });
      }

      // 2. Enforce school boundary verification for multi-tenancy
      if (req.user.role !== 'superadmin') {
        if (!schoolId || schoolId !== req.user.schoolId) {
          return res.status(403).json({ error: 'FORBIDDEN', message: '╪║┘è╪▒ ┘à╪│┘à┘ê╪¡ ┘ä┘â ╪¿╪Ñ┘å╪┤╪º╪í ┘à╪│╪¬╪«╪»┘à┘è┘å ┘ä┘à╪»╪▒╪│╪⌐ ╪ú╪«╪▒┘ë' });
        }
      }
        ...
        if (userDoc.exists) {
          ...
          if (role === 'parent') {
            if (existingUserData?.role && existingUserData.role !== 'parent') {
              return res.status(400).json({
                error: 'ROLE_CONFLICT',
                message: '┘ç╪░╪º ╪º┘ä╪¿╪▒┘è╪» ╪º┘ä╪Ñ┘ä┘â╪¬╪▒┘ê┘å┘è ┘à╪│╪¼┘ä ╪¿╪¡╪│╪º╪¿ ┘à┘ê╪╕┘ü ╪ú┘ê ┘à╪╣┘ä┘à. ┘è╪▒╪¼┘ë ╪º╪│╪¬╪«╪»╪º┘à ╪¿╪▒┘è╪» ┘à╪«╪¬┘ä┘ü ┘ä┘ê┘ä┘è ╪º┘ä╪ú┘à╪▒.'
              });
            }
          } else {
            ...
            if (existingUserData?.schoolId && existingUserData.schoolId !== schoolId && req.user.role !== 'superadmin') {
              return res.status(400).json({ 
                ...
                message: '┘ç╪░╪º ╪º┘ä╪¿╪▒┘è╪» ╪º┘ä╪Ñ┘ä┘â╪¬╪▒┘ê┘å┘è ┘à╪│╪¼┘ä ╪¿╪º┘ä┘ü╪╣┘ä ┘ä┘à╪»╪▒╪│╪⌐ ╪ú╪«╪▒┘ë' 
              });
            }
          }
        }
      ...
      if (role === 'student' && schoolId) {
        ...
        await db.runTransaction(async (transaction) => {
          ...
          if (!schoolSnap.exists) throw new Error('╪º┘ä┘à╪»╪▒╪│╪⌐ ╪║┘è╪▒ ┘à┘ê╪¼┘ê╪»╪⌐');
          ...
          if (shouldIncrement && currentCount >= maxStudents) {
            throw new Error(`┘ê╪╡┘ä╪¬ ┘ä┘ä╪¡╪» ╪º┘ä╪ú┘é╪╡┘ë ╪º┘ä┘à╪│┘à┘ê╪¡ ╪¿┘ç ┘ä┘ä╪╖┘ä╪º╪¿ (${maxStudents})`);
          }
      ...
      res.status(500).json({
        ...
        message: error.message || '╪¡╪»╪½ ╪«╪╖╪ú ╪ú╪½┘å╪º╪í ╪Ñ┘å╪┤╪º╪í ╪º┘ä┘à╪│╪¬╪«╪»┘à'
      });
    }
  });
  ...
  app.post('/api/admin/sync-claims', verifyAdmin, async (req: any, res: any) => {
    ...
    try {
      ...
      const allowedAdminRoles = ['superadmin', 'admin', 'staff', 'assistant'];
      if (!allowedAdminRoles.includes(req.user.role)) {
        return res.status(403).json({ error: 'FORBIDDEN', message: '╪║┘è╪▒ ┘à╪╡╪▒╪¡ ┘ä┘â ╪¿╪º┘ä┘é┘è╪º┘à ╪¿┘à╪▓╪º┘à┘å╪⌐ ╪º┘ä╪╡┘ä╪º╪¡┘è╪º╪¬' });
      }
      ...
      if (req.user.role !== 'superadmin') {
        if (!schoolId || schoolId !== req.user.schoolId) {
          return res.status(403).json({ error: 'FORBIDDEN', message: '╪║┘è╪▒ ┘à╪│┘à┘ê╪¡ ┘ä┘â ╪¿┘à╪▓╪º┘à┘å╪⌐ ╪╡┘ä╪º╪¡┘è╪º╪¬ ╪ú╪╣╪╢╪º╪í ╪º┘ä┘à╪»╪º╪▒╪│ ╪º┘ä╪ú╪«╪▒┘ë' });
        }
      }
  ...
  app.post('/api/admin/delete-user', verifyAdmin, async (req: any, res: any) => {
    ...
    try {
      ...
      const allowedAdminRoles = ['superadmin', 'admin', 'staff', 'assistant'];
      if (!allowedAdminRoles.includes(req.user.role)) {
        return res.status(403).json({ error: 'FORBIDDEN', message: '╪║┘è╪▒ ┘à╪╡╪▒╪¡ ┘ä┘â ╪¿╪¡╪░┘ü ╪¡╪│╪º╪¿╪º╪¬ ╪º┘ä┘à╪│╪¬╪«╪»┘à┘è┘å' });
      }

      const db = getDb();
      const userDoc = await db.collection('users').doc(uid).get();
      if (!userDoc.exists) {
        return res.status(404).json({ error: 'USER_NOT_FOUND', message: '╪º┘ä┘à╪│╪¬╪«╪»┘à ╪║┘è╪▒ ┘à┘ê╪¼┘ê╪»' });
      }
      ...
      if (req.user.role !== 'superadmin') {
        if (!targetSchoolId || targetSchoolId !== req.user.schoolId) {
          return res.status(403).json({ error: 'FORBIDDEN', message: '╪║┘è╪▒ ┘à╪╡╪▒╪¡ ┘ä┘â ╪¿╪¡╪░┘ü ┘à╪│╪¬╪«╪»┘à┘ë ╪º┘ä┘à╪»╪º╪▒╪│ ╪º┘ä╪ú╪«╪▒┘ë' });
        }
      }
      ...
      res.status(500).json({
        success: false,
        error: error.message || 'Internal Server Error',
        message: error.message || '╪¡╪»╪½ ╪«╪╖╪ú ╪ú╪½┘å╪º╪í ╪¡╪░┘ü ╪º┘ä╪¡╪│╪º╪¿'
      });
    }
  });
  ...
  app.post('/api/admin/delete-school', verifyAdmin, async (req: any, res: any) => {
    ...
    try {
      ...
      if (req.user.role !== 'superadmin') {
        return res.status(403).json({ error: 'FORBIDDEN', message: '╪║┘è╪▒ ┘à╪╡╪▒╪¡ ┘ä┘â ╪¿╪¡╪░┘ü ╪º┘ä┘à╪»╪▒╪│╪⌐. ┘ç╪░╪º ╪º┘ä╪Ñ╪¼╪▒╪º╪í ┘à╪«╪╡╪╡ ┘ä┘ä┘à╪╖┘ê╪▒/SuperAdmin ┘ü┘é╪╖' });
      }
  ...
  app.post('/api/admin/delete-student', verifyAdmin, async (req: any, res: any) => {
    ...
    try {
      ...
      const allowedAdminRoles = ['superadmin', 'admin', 'staff', 'assistant'];
      if (!allowedAdminRoles.includes(req.user.role)) {
        return res.status(403).json({ error: 'FORBIDDEN', message: '╪║┘è╪▒ ┘à╪╡╪▒╪¡ ┘ä┘â ╪¿╪¡╪░┘ü ╪º┘ä╪╖┘ä╪º╪¿' });
      }
      ...
      if (req.user.role !== 'superadmin') {
        if (!schoolId || schoolId !== req.user.schoolId) {
          return res.status(403).json({ error: 'FORBIDDEN', message: '╪║┘è╪▒ ┘à╪│┘à┘ê╪¡ ┘ä┘â ╪¿╪¡╪░┘ü ╪╖┘ä╪º╪¿ ┘à╪»╪▒╪│╪⌐ ╪ú╪«╪▒┘ë' });
        }
      }
      ...
      res.status(500).json({
        success: false,
        error: error.message || 'Internal Server Error',
        message: error.message || '╪¡╪»╪½ ╪«╪╖╪ú ╪ú╪½┘å╪º╪í ╪¡╪░┘ü ╪º┘ä╪╖╪º┘ä╪¿'
      });
    }
  });
  ...
  app.all('/api/*', (req, res) => {
    ...
    res.status(404).json({
      ...
      message: '╪º┘ä╪╣┘å┘ê╪º┘å ╪║┘è╪▒ ┘à┘ê╪¼┘ê╪» ╪ú┘ê ╪║┘è╪▒ ┘à╪╣╪▒┘ü ┘ü┘è ╪º┘ä┘å╪╕╪º┘à'
    });
  });
  ...
  app.use('/api/*', (err: any, req: any, res: any, next: any) => {
    ...
    res.status(statusCode).json({
      ...
      message: err.message || '╪¡╪»╪½ ╪«╪╖╪ú ╪»╪º╪«┘ä┘è ┘ü┘è ╪º┘ä╪«╪º╪»┘à'
    });
  });
