...
/**
 * تحميل iPhone — App Store إن وُجد، وإلا ملف تعريف ثابت (يعمل على Hostinger بدون API).
 */
export function FooterIosDownload({ appName = 'SchoolixiQ', compact = false }: Props) {
  ...
  const profileUrl = getIosMobileConfigUrl(config);
  const href = storeUrl || profileUrl;
  ...
  const handleClick = (e: MouseEvent) => {
    ...
    // ملف .mobileconfig: اترك المتصفح يفتح الرابط (لا ?install=ios ولا SPA refresh)
  };
