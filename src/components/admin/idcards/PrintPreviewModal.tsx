  ...
  return (
    <>
      <AppModalPortal
        ...
        dir={isRtl ? 'rtl' : 'ltr'}
        size="xl"
        ariaLabel={isRtl ? 'إعدادات الطباعة' : 'Print Settings'}
      >
        <div className="print:hidden flex flex-col md:flex-row overflow-hidden min-h-0 flex-1 h-[min(85vh,820px)]">
          <div className="sx-app-modal-panel__header md:hidden shrink-0">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400 font-bold rounded-xl flex items-center justify-center">
                <Printer size={20} />
              </div>
              <div>
            ...
            <button type="button" className="sx-app-modal-panel__close" onClick={onClose} aria-label={isRtl ? 'إغلاق' : 'Close'}>
              <X size={18} />
            </button>
          </div>
          ...
          {/* Preview Area */}
          <div className="flex-1 bg-slate-200 dark:bg-slate-950 p-4 md:p-8 flex flex-col items-center overflow-auto custom-scrollbar relative min-h-0">
            <button
              onClick={onClose}
              className="hidden md:flex absolute top-6 right-6 p-2 bg-slate-100 dark:bg-slate-800 rounded-full text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 transition-colors z-20"
      ...
      <AppModalPortal
        ...
        onClose={() => setIsConfirmingPrint(false)}
        dir={isRtl ? 'rtl' : 'ltr'}
        size="sm"
        ariaLabel={isRtl ? 'تأكيد الطباعة' : 'Confirm print'}
      >
        <div className="print:hidden">
          <div className="sx-app-modal-panel__header">
            ...
            <button type="button" className="sx-app-modal-panel__close" onClick={() => setIsConfirmingPrint(false)} aria-label={isRtl ? 'إغلاق' : 'Close'}>
              <X size={18} />
            </button>
          </div>
