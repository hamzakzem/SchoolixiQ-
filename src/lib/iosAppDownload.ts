...
const MOBILECONFIG_PATH = '/api/download/schoolixiq.mobileconfig';
...
export function triggerIosWebClipInstall(): void {
  ...
  window.location.assign(MOBILECONFIG_PATH);
}
...
export function openIosInstall(config?: IosDownloadConfig | null): boolean {
  ...
  const landing = getIosInstallLandingUrl(config);
