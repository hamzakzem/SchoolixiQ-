...
export default function Announcements() {
  ...
  const studentSearchAnchorRef = useRef<HTMLDivElement>(null);
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      >
        <div className="sx-app-modal-panel__body text-center space-y-4">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-bold text-sm">هل أنت متأكد من رغبتك في حذف هذا الإعلان؟ لا يمكن التراجع عن هذا الإجراء.</p>
        </div>
        <div className="sx-app-modal-panel__footer flex gap-4">
          <button
            type="button"
            onClick={() => setDeleteConfirmId(null)}
            className="flex-1 py-3 px-4 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-xl font-bold hover:bg-slate-200 transition-all min-h-12"
          >
            إلغاء
          </button>
          <button
            ...
            className="flex-1 py-3 px-4 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 transition-all shadow-lg shadow-red-600/20 min-h-12"
          >
            حذف الآن
          </button>
        </div>
      ...
      >
        <div className="sx-app-modal-panel__header">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="sx-app-modal-panel__title">نشر تعميم / تبليغ</h2>
              <p className="sx-app-modal-panel__subtitle">رسالة تواصل جماعية — ليست تقييماً دراسياً ولا حادثة سلوكية</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleAdd} className="sx-app-modal-panel__body space-y-6">
            ...
            {newAnnouncement.target === 'individual' && (
              <div className="md:col-span-2">
                ...
                {newAnnouncement.targetStudentId ? (
                  <div className="flex items-center justify-between p-4 bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800/50 rounded-2xl">
                  ...
                  <div ref={studentSearchAnchorRef} className="relative group">
                    ...
                    <input 
                      type="text"
                      className="w-full pr-12 pl-6 py-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800 text-slate-900 dark:text-white font-bold outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all"
                    ...
                    <FloatingMenu
                      open={showStudentSearch}
                      onClose={() => {
                        setShowStudentSearch(false);
                        setSelectedGrade(null);
                      }}
                      anchorRef={studentSearchAnchorRef}
                      role="listbox"
                      align="start"
                      ...
                      {browsingMode === 'search' ? (
                        <>
                          ...
                          {filteredStudents.length > 0 ? (
                            <div className="divide-y divide-slate-50 dark:divide-slate-800/50" role="listbox">
          ...
          <button 
            type="submit"
            disabled={loading}
            className="w-full py-5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-[1.5rem] font-bold text-lg hover:shadow-2xl transition-all active:scale-95 disabled:opacity-50 mt-4 flex items-center justify-center gap-3 shadow-xl min-h-12"
          >
            <Send size={24} />
